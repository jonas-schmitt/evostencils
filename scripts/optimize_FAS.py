from evostencils.optimization.program_FAS import OptimizerFAS
...
def main():
    ...
    program_generator_fas = ProgramGeneratorFAS('FAS_2D_Basic', 'Solution', 'RHS', 'Residual', 'Approximation',
                                                'RestrictionNode', 'CorrectionNode',
                                                'Laplace', 'gamSten', 'mgCycle', max_level, min_level, 'CGS', 'Smoother')

    # Create optimizer object
    optimizer = OptimizerFAS(dimension, finest_grid, coarsening_factors, min_level, max_level, equations, operators, fields,
                             mpi_comm=comm, mpi_rank=mpi_rank, number_of_mpi_processes=nprocs,
                             program_generator=program_generator_fas,
    ...
    mu_ = 4  # Population size # 16 for single objective (64 for the final report)
    lambda_ = 4  # Number of offspring # 16 for single objective (64 for the final report)
    generations = 1  # Number of generations # 20 generations
    population_initialization_factor = 1  # Multiply mu_ by this factor to set the initial population size # 2
