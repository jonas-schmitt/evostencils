...
class RooflineEvaluator:
    ...
    def estimate_correction_performance_metrics(self, correction: multigrid.Correction) -> list:
        iteration_matrix = correction.iteration_matrix
        evaluated, list_of_metrics = self.estimate_iteration_performance_metrics(iteration_matrix)
    ...
    def estimate_iteration_performance_metrics(self, expression: base.Expression) -> tuple:
        if isinstance(expression, base.BinaryExpression):
            evaluated1, result1 = self.estimate_iteration_performance_metrics(expression.operand1)
            evaluated2, result2 = self.estimate_iteration_performance_metrics(expression.operand2)
            metrics = []
            ...
            result = self.estimate_iteration_performance_metrics(expression.operand)
            if result[0]:
